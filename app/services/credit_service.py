...
def add_credits(
    ...
    centre_id: int,
    amount: int,
    transaction_type: models.TransactionTypeEnum,
    user_id: int = None,
    reference_type: str = None,
    reference_id: int = None,
    description: str = None
) -> int:
    query = db.query(models.CentreCreditAccount).filter(
        models.CentreCreditAccount.centre_id == centre_id
    )
    # Use row-level locking on Postgres, skip syntax error on SQLite
...
def deduct_credit_locked(
    ...
    tx = models.CreditTransaction(
        ...
        amount=-amount,
        balance_after=account.balance,
        transaction_type=models.TransactionTypeEnum.report_upload,
        reference_type=reference_type,
        reference_id=reference_id,
        description=description,
        created_at=datetime.utcnow()
