...
worker_code = """from datetime import datetime, timezone, timedelta
...
class BackgroundWorker:
        ...
        for attempt in range(10):
            try:
                ...
                if result.rowcount == 1:
                    ...
                    db.rollback()
                    # Another worker claimed it; retry to find next available job
                    continue
    ...
    def process_job(self, db: Session, job: NotificationJob, provider=None) -> str:
        ...
        try:
            if provider:
                success = provider.send(job.channel, job.recipient, job.message_payload)
                if not success:
...
print("Background worker updated with robust atomic claim loop.")
